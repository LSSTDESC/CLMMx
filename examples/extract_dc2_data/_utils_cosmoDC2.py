...
def _fix_axis_ratio(q_bad):
    ...
    q_new = np.sqrt(
        (1 - e_jb) / (1 + e_jb)
    )  # use correct relationship to compute q from e_jb
    return q_new
...
def correct_shear_ellipticity(ellipticity_uncorr_e1, ellipticity_uncorr_e2):
    ...
    # from https://github.com/LSSTDESC/gcr-catalogs/blob/ellipticity_bug_fix/GCRCatalogs/cosmodc2.py
    ellipticity_uncorr_norm = (
        ellipticity_uncorr_e1**2 + ellipticity_uncorr_e2**2
    ) ** 0.5
    ...
    ellipticity_corr_e1, ellipticity_corr_e2 = (
        ellipticity_corr.real,
        ellipticity_corr.imag,
    )
...
def extract_cosmoDC2_galaxy(lens_z, lens_distance, ra, dec, rmax=10, method="qserv"):
    ...
    if method == "qserv":
        ...
        def qserv_query(lens_z, lens_distance, ra, dec, rmax=10):
            ...
            zmax = 3.0
            zmin = lens_z
            theta_max = (rmax / lens_distance) * (180.0 / np.pi)
        ...
        conn_qserv = mysql.connector.connect(
            host="ccqserv201", user="qsmaster", port=30040
        )
    ...
    if method == "gcrcatalogs":
        ...
        healpix_dc2 = GCRCatalogs.load_catalog(
            "cosmoDC2_v1.1.4_image"
        ).get_catalog_info()["healpix_pixels"]
        ...
        for i, hp in enumerate(healpix_list):
            ...
            chunk = cosmoDC2.get_quantities(
                gcrcatalogs_query(),
                filters=(coord_filters + z_filters + mag_filters),
                native_filters=[f"healpix_pixel=={hp}"],
