...
def collect_argparser():
    parser = argparse.ArgumentParser(
        description="Let's extract source catalogs and estimate individual lensing profiles"
    )
    ...
    parser.add_argument(
        ...
        type=str,
        required=False,
        default="./DC2_data/cosmoDC2_v1.1.4_redmapper_v0.8.1_catalog.pkl",
    )
    parser.add_argument(
        "--lambda_min",
        type=float,
...
mask_select *= (
    lens_catalog["redshift"] > _config_extract_sources_in_cosmoDC2.redshift_min
)
mask_select *= (
    lens_catalog["redshift"] < _config_extract_sources_in_cosmoDC2.redshift_max
)
...
split_lists = np.array_split(
    index_cl, int(_config_extract_sources_in_cosmoDC2.number_of_splits)
)
lens_catalog_truncated = lens_catalog_truncated[
    np.array(split_lists[int(_config_extract_sources_in_cosmoDC2.which_split)])
]
n_cl_to_extract = len(lens_catalog_truncated)

path_DC2_data_ccin2p3 = "/sps/lsst/groups/clusters/CLMM/"
path_DC2_data_nersc = "/global/cfs/projectdirs/lsst/groups/CL/CLMM/"
path_CLMM_data = path_DC2_data_nersc

path_where_to_save_profiles = (
    path_CLMM_data + f"./DC2_data/individual_redMaPPer_cluster_lensing_profiles/"
)
...
print(
    f"[cluster catalog]: {_config_extract_sources_in_cosmoDC2.lambda_min} < richness < {_config_extract_sources_in_cosmoDC2.lambda_max}"
)
print(
    f"[cluster catalog]: {_config_extract_sources_in_cosmoDC2.redshift_min} < redshift < {_config_extract_sources_in_cosmoDC2.redshift_max}"
)
print(f"[cluster catalog]: number of clusters = {n_cl}")
print(
    f"[cluster catalog]: number of clusters in split {_config_extract_sources_in_cosmoDC2.which_split} = {n_cl_to_extract}"
)
...
for n, lens in enumerate(lens_catalog_truncated):
    ...
    tab = _utils_cosmoDC2.extract_cosmoDC2_galaxy(
        z,
        lens_distance,
        ra,
        dec,
        rmax=_config_extract_sources_in_cosmoDC2.rmax,
        method=_config_extract_sources_in_cosmoDC2.method,
    )
    # compute reduced shear and ellipticities
    tab["g1"], tab["g2"] = clmm.utils.convert_shapes_to_epsilon(
        tab["shear1"], tab["shear2"], shape_definition="shear", kappa=tab["kappa"]
    )
    ellipticity_uncorr_e1 = tab["e1_true_uncorr"]
    ellipticity_uncorr_e2 = tab["e2_true_uncorr"]
    ellipticity_corr_e1, ellipticity_corr_e2 = (
        _utils_cosmoDC2.correct_shear_ellipticity(
            ellipticity_uncorr_e1, ellipticity_uncorr_e2
        )
    )
    tab["e1_true"] = ellipticity_corr_e1
    tab["e2_true"] = ellipticity_corr_e2
    tab["e1"], tab["e2"] = clmm.utils.compute_lensed_ellipticity(
        tab["e1_true"], tab["e2_true"], tab["shear1"], tab["shear2"], tab["kappa"]
    )
    ...
    bin_edges = clmm.dataops.make_bins(
        0.5, _config_extract_sources_in_cosmoDC2.rmax, 15, method="evenlog10width"
    )
